...
import { listTickets, getTicket, createTicket, updateTicket, listComments, addComment, } from './db.js';
...
let sseEmitter;
export function setSseEmitter(fn) {
    sseEmitter = fn;
}
    ...
    async register(app, _nc, _manager, opts) {
        ...
        app.patch('/api/tickets/:id', (req, res) => {
            try {
                ...
                if (!existing)
                    return res.status(404).json({ error: 'Ticket not found' });
                const changedBy = req.body.changed_by ?? 'api';
                ...
                if (newStatus && newStatus !== existing.status) {
                    ...
                    if (topic) {
                        ...
                        void publishNats(topic, payload).catch((e) => console.error(`[dev-team plugin] Failed to publish ${topic}:`, e));
                        console.log(`[dev-team plugin] Published NATS: ${topic} for ${id}`);
                    }
                    if (newStatus === 'done')
                        emit('ticket:done', { id, ticket: updated });
                }
                res.json(updated);
            }
            catch (err) {
