...
export function nextTicketId() {
    ...
    const row = database.prepare('SELECT COUNT(*) as cnt FROM tickets').get();
...
export function getTicket(id) {
    return openDb().prepare('SELECT * FROM tickets WHERE id = ?').get(id);
}
...
export function updateTicket(id, data, changedBy) {
    const database = openDb();
    ...
    const fields = ['title', 'status', 'priority', 'type', 'parent_id', 'blocked_by', 'author', 'assigned_to', 'labels', 'body', 'model_hint'];
...
export function listComments(ticketId) {
    return openDb().prepare('SELECT * FROM ticket_comments WHERE ticket_id = ? ORDER BY created_at ASC').all(ticketId);
}
export function addComment(ticketId, author, body) {
    const database = openDb();
    const now = new Date().toISOString().replace('T', ' ').replace(/\.\d+Z$/, '');
    ...
    return database.prepare('SELECT * FROM ticket_comments WHERE id = ?').get(result.lastInsertRowid);
}
